  ...
  let
    contentLength =
      Text.encodeUtf8
        ...
        $ ByteString.length x
    headers =
      (Http.hContentLength, contentLength) : h
  in Wai.responseLBS s headers
    $ LazyByteString.fromStrict x
...
lbs
  ...
  -> Wai.Response
lbs s h =
  bs s h . LazyByteString.toStrict
